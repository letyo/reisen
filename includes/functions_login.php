<?php
	...
	function login($username, $password, $conn) {
		...
		$stmt = $conn->prepare("SELECT user_id, username, password, email, next_valid_login_time FROM " . TABLE_NAME_REISEN__MEMBERS . " WHERE username = ?");
		...
		if (bruteforce($db_user_id, $conn) == true) {
			...
			$email = "Riesen.Reisen@mail.com"; //a webhely üzemeltető email címe
			date_default_timezone_set("Europe/Vienna"); //az időzóna
			$next_login = date("d.m.Y. H:i:s", strtotime($db_next_valid_login_time)); //az időt a server időzónájában írja ki!!!!!!!!!!!!!!!!!!!!!!!!!!
			$locked_time = date("d.m.Y. H:i:s", time()); //az időt a server időzónájában írja ki!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	function bruteforce($user_id, $conn) {
		...
		$stmt = $conn->prepare("SELECT time FROM " . TABLE_NAME_REISEN__BRUTE . " WHERE user_id = ? AND time > ?");
		...
		$stmt = $conn->prepare("SELECT next_valid_login_time FROM " . TABLE_NAME_REISEN__MEMBERS . " WHERE user_id = ? AND next_valid_login_time > ?");
		...
		if ($num_rows_brute > 5) {
			...
			if ($num_rows_members == 0) {
				...
				$stmt = $conn->prepare("UPDATE " . TABLE_NAME_REISEN__MEMBERS . " SET next_valid_login_time = ? WHERE user_id = ?");
	...
	function faild_login_attempts($user_id, $conn) {
		...
		if ($stmt->num_rows == 0) {
			...
			$stmt = $conn->prepare("INSERT INTO " . TABLE_NAME_REISEN__BRUTE . "(user_id, time) VALUES(?, ?)");
			$stmt->bind_param("ii", $user_id, time());
			$stmt->execute();
			$stmt->close();
		}
	...
	function stay_logged_in($user_id, $conn) {
		...
		$stmt = $conn->prepare("UPDATE " . TABLE_NAME_REISEN__MEMBERS . " SET session_id = ? WHERE user_id = ?");
